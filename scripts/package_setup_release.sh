...
#
# Writes: dist/the-fifth-element-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix the-fifth-element \
  --exe-name The_Fifth_Element_Recompiled \
  --display-name "The Fifth Element Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --runtime-dir mods \
  --disc-hint "Fifth Element, The (Europe) (En,Fr,De,Es,It).cue" \
